...
export default function ViewerProjectsPage() {
  ...
  useEffect(() => {
    trackEvent('projects_viewed', { role: 'viewer' });
    loadProjects();
  }, []);
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => (
          <Card
            ...
            onClick={() => router.push(`/viewer/projects/${toSlug(project.name, project.id)}`)}
          >
            <CardHeader>
      ...
      {projects.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted-foreground">No projects available to view.</p>
        </div>
      )}
    </div>
  );
